...
export default function SignInPage() {
  ...
  return <main className="sign-in-page"><header className="site-header"><Link className="brand" href="/"><span className="brand-line" />Meander</Link><nav aria-label="Main navigation"><Link href="/how-it-works">How Meander works</Link><Link href="/gallery">Gallery</Link></nav><Link className="nav-cta" href="/create">Create first <span>↗</span></Link></header><section className="sign-in-card"><p className="section-kicker">Save your work</p><h1>Sign in to<br />keep moving.</h1><p>Use your Google account to save your walks, keep artwork private, and choose when to share or publish it.</p><div className="google-sign-in" ref={button} aria-label="Continue with Google" />{busy && <div className={`auth-progress ${authState === "success" ? "complete" : ""}`} role="status"><span>{authState === "success" ? "✓" : ""}</span><div><strong>{authState === "checking" ? "Checking your session…" : authState === "signing-in" ? "Creating your Meander account…" : "You’re signed in."}</strong><p>{authState === "success" ? "Taking you to the creation studio." : "This should only take a moment."}</p></div></div>}{error && <p className="engine-error" role="alert">{error}</p>}{configurationLoaded && !clientID && !error && <p className="engine-error" role="alert">Google Sign-In is still being connected. Please try again shortly.</p>}<small>Meander only uses your Google identity to create your account. Raw route files and screenshots are processed for the creation and are not stored with your account.</small></section></main>;
}
